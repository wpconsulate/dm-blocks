...
class DMBlockWidget extends WP_Widget {

    private $num = 5;
    private $domain = "dm-blocks";

    function __construct() {
        $widget_ops = array(
            'description' => __('Display Selected DB Block.', $this->domain)
        );
        $control_ops = array('width' => 400, 'height' => 350);
        parent::__construct(
            false,
            __('DM Block', $this->domain),
            $widget_ops,
            $control_ops
        );
    }
    ...
    public function form($instance) {
        ...
        echo '<p>';
        echo __("DM Block Post ID", $this->domain);
        echo "<br />";
    ...
    public function widget($args, $instance) {
        ...
        $pid = apply_filters('dm_block_widget_post_id', $pid);
        
        echo do_shortcode('[dm_block id="'.$pid.'"]');
        
    }
    public function widget_extended($args, $instance) {
        ...
        $p = apply_filters('dm_block_widget_post', get_post($pid));
    ...
    private function template()
    {
        ...
        $html .= '</div>';
        return apply_filters("dm-blocks-template", $html);
    }
}

class DMBlockWidgetInit {
...
{
    $url = plugins_url("", __FILE__).'/assets/css/style.css';
    printf(
        '<link rel="stylesheet" type="text/css" media="all" href="%s" />'."\n",
        apply_filters("dm-blocks-stylesheet", $url)
    );
}

public function widgets_init()
{
    return register_widget("DMBlockWidget");
}
